...
function buildInsertPlan(columns, recruiterId, job) {
  ...
  const candidate = {
    ...
    application_link: truncateField(job.applicationLink || ''),
    company_name: truncateField(job.companyName),
    how_to_apply: truncateField(job.howToApply || ''),
    job_type: truncateField(job.jobType || ''),
    work_type: truncateField(job.workType || ''),
    required_skills: truncateField(job.requiredSkills || ''),
